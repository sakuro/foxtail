...
RSpec.describe Fantail::Resource do
  describe ".from_string" do
    ...
    it "parses FTL source into a Resource" do
      resource = Fantail::Resource.from_string(ftl_source)

      expect(resource).to be_a(Fantail::Resource)
    ...
    it "creates proper message entries" do
      ...
      expect(hello_msg).to be_a(Fantail::Bundle::Parser::AST::Message)
      expect(hello_msg.id).to eq("hello")
      expect(hello_msg.value).to be_an(Array)

      goodbye_msg = resource.entries[2]
      expect(goodbye_msg).to be_a(Fantail::Bundle::Parser::AST::Message)
    ...
    it "creates proper term entries" do
      ...
      expect(brand_term).to be_a(Fantail::Bundle::Parser::AST::Term)
  ...
  describe ".from_file" do
    ...
    it "reads and parses FTL file" do
      resource = Fantail::Resource.from_file(Pathname(temp_file.path))
  ...
  describe "instance methods" do
    ...
    describe "#empty?" do
      ...
      it "returns true when resource is empty" do
        empty_resource = Fantail::Resource.from_string("")
    ...
    describe "#messages" do
      it "returns only message entries" do
        ...
        expect(messages.all?(Fantail::Bundle::Parser::AST::Message)).to be true
      end
    end
    ...
    describe "#terms" do
      it "returns only term entries" do
        ...
        expect(terms.all?(Fantail::Bundle::Parser::AST::Term)).to be true
      end
    end
    ...
    describe "#find" do
      it "finds entry by ID" do
        ...
        expect(entry).to be_a(Fantail::Bundle::Parser::AST::Message)
        expect(entry.id).to eq("hello")
      end

      it "finds term entry by ID" do
        entry = resource.find("-brand")
        expect(entry).to be_a(Fantail::Bundle::Parser::AST::Term)
