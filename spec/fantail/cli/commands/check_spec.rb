...
RSpec.describe Fantail::CLI::Commands::Check do
  let(:cli) { Dry.CLI(Fantail::CLI::Commands::Check.new) }
    ...
    context "with invalid FTL files" do
      it "raises CheckError with error count" do
        Tempfile.create(%w[invalid .ftl]) do |f|
          ...
          expect {
            cli.call(arguments: [f.path, "--quiet"], out:, err:)
          }.to raise_error(Fantail::CLI::CheckError) {|e| expect(e.error_count).to eq(1) }
        end
      end
      ...
      it "outputs junk content with file path to stderr" do
        Tempfile.create(%w[invalid .ftl]) do |f|
          ...
          expect {
            cli.call(arguments: [f.path, "--quiet"], out:, err:)
          }.to raise_error(Fantail::CLI::CheckError)
