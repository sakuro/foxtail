...
RSpec.describe Fantail::Syntax::Parser::Stream do
  describe "basic functionality" do
    let(:stream) { Fantail::Syntax::Parser::Stream.new("hello world") }
  ...
  describe "CRLF handling" do
    let(:stream) { Fantail::Syntax::Parser::Stream.new("line1\r\nline2\nline3") }
  ...
  describe "blank handling" do
    let(:stream) { Fantail::Syntax::Parser::Stream.new("  hello   \n  world") }
  ...
  describe "character classification" do
    let(:stream) { Fantail::Syntax::Parser::Stream.new("a1-_Z9") }
  ...
  describe "number handling" do
    let(:stream) { Fantail::Syntax::Parser::Stream.new("123-456") }
    ...
    it "handles negative numbers" do
      stream = Fantail::Syntax::Parser::Stream.new("-123")
      expect(stream.number_start?).to be true
    end

    it "rejects non-number starting with dash" do
      stream = Fantail::Syntax::Parser::Stream.new("-abc")
  ...
  describe "EOF handling" do
    let(:stream) { Fantail::Syntax::Parser::Stream.new("a") }

    it "handles EOF correctly" do
      expect(stream.current_char).to eq("a")
      stream.next
      expect(stream.current_char).to be_nil # EOF
      expect(stream.current_char).to eq(Fantail::Syntax::Parser::Stream::EOF)
    end
  end

  describe "constants" do
    it "defines correct constants" do
      expect(Fantail::Syntax::Parser::Stream::EOL).to eq("\n")
      expect(Fantail::Syntax::Parser::Stream::EOF).to be_nil
      expect(Fantail::Syntax::Parser::Stream::SPECIAL_LINE_START_CHARS).to eq(["}", ".", "[", "*"])
    end
  end
  ...
  describe "error handling" do
    ...
    it "raises ParseError for expect_char mismatch" do
      expect { stream.expect_char("x") }.to raise_error(Fantail::Syntax::Parser::ParseError, 'Expected token: "x"')
    end
    ...
    it "raises ParseError for take_id_start with invalid character" do
      stream = Fantail::Syntax::Parser::Stream.new("123")
      expect { stream.take_id_start }.to raise_error(Fantail::Syntax::Parser::ParseError, 'Expected a character from range: "a-zA-Z"')
    end
  end
end
