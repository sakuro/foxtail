...
module Fantail
  # Command-line interface for Fantail
  module CLI
    module Commands
      ...
      class Tidy < Dry::CLI::Command
        ...
        def call(files:, write:, check:, diff:, with_junk:, **)
          raise Fantail::CLI::NoFilesError if files.empty?
          ...
          raise Fantail::CLI::TidyError, files_with_errors unless files_with_errors.empty?
          raise Fantail::CLI::TidyCheckError, files_needing_format if check && !files_needing_format.empty?
        end
        ...
        private def process_file(path, write:, check:, diff:, with_junk:, multiple_files:)
          ...
          has_junk = resource.body.any?(Fantail::Syntax::Parser::AST::Junk)
          return :has_errors if has_junk && !with_junk

          serializer = Fantail::Syntax::Serializer.new(with_junk:)
