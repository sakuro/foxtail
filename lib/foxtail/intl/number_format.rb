...
module Foxtail
  module Intl
    ...
    class NumberFormat
      ...
      #   formatter = Foxtail::Intl::NumberFormat.new(locale: Locale::Tag.parse("en-US"))
      ...
      def call(value)
        ...
        has_percent = tokens.any?(PatternParser::Number::PercentToken)
        has_permille = tokens.any?(PatternParser::Number::PerMilleToken)
        style = @options[:style] || "decimal"
      ...
      private def use_currency?(options)
        ...
        if pattern
          parser = PatternParser::Number.new
          tokens = parser.parse(pattern)
          return tokens.any?(PatternParser::Number::CurrencyToken)
        end
      ...
      private def build_formatted_string(format_value, tokens, original_was_negative)
        ...
        has_scientific = tokens.any?(PatternParser::Number::ExponentToken)
        ...
        pattern_info[:suffix_tokens].each do |token|
          result += if has_scientific && token.is_a?(PatternParser::Number::ExponentToken)
      ...
      private def analyze_pattern_structure(tokens)
        ...
        tokens.each do |token|
          case token
          when PatternParser::Number::DigitToken
          ...
          when PatternParser::Number::GroupToken
            if digit_section_started && !decimal_found
              info[:has_grouping] = true
              info[:integer_pattern] << token
            end
          when PatternParser::Number::DecimalToken
            decimal_found = true
      ...
      private def format_non_digit_token(token, decimal_value)
        ...
        when PatternParser::Number::LiteralToken
          token.value
        when PatternParser::Number::QuotedToken
          token.literal_text
      ...
      private def combine_patterns_using_tokens(number_pattern, style_pattern)
        ...
        style_tokens.each do |token|
          case token
          when PatternParser::Number::DigitToken,
               PatternParser::Number::DecimalToken,
               PatternParser::Number::GroupToken,
               PatternParser::Number::ExponentToken
      ...
      private def apply_compact_pattern(decimal_value, compact_info)
        ...
        digit_tokens = tokens.select {|t| t.is_a?(PatternParser::Number::DigitToken) }
      ...
      private def apply_compact_pattern_using_tokens(tokens, formatted_number)
        ...
        tokens.each do |token|
          ...
          when PatternParser::Number::LiteralToken,
               PatternParser::Number::QuotedToken

            # Keep literal tokens (including unit symbols like "K", "万")
            result_parts << if token.is_a?(PatternParser::Number::QuotedToken)
                              token.literal_text
                            else
                              token.to_s
                            end
      ...
      private def extract_unit_symbol_from_pattern(pattern)
        ...
        unit_tokens = tokens.reject {|token|
          token.is_a?(PatternParser::Number::DigitToken)
        }
        ...
        if is_negative
          ...
          has_separator_in_prefix =
            ...
            pattern_info[:prefix_tokens].any? {|t|
              !t.is_a?(PatternParser::Number::CurrencyToken) &&
                !t.is_a?(PatternParser::Number::PercentToken)
            }
      ...
      private def format_compact_non_digit_token(token, style_symbol)
        case token
        when PatternParser::Number::CurrencyToken,
             PatternParser::Number::PercentToken

          style_symbol
        when PatternParser::Number::LiteralToken,
             PatternParser::Number::QuotedToken

          token.is_a?(PatternParser::Number::QuotedToken) ? token.literal_text : token.to_s
        else
          ""
        end
      end
      ...
      private def format_special_value(value)
        ...
        pattern_tokens.each do |token|
          break if token.is_a?(PatternParser::Number::DigitToken) ||
                   token.is_a?(PatternParser::Number::GroupToken) ||
                   token.is_a?(PatternParser::Number::DecimalToken)

          # Skip exponent tokens for special values - they don't have meaningful exponents
          next if token.is_a?(PatternParser::Number::ExponentToken)
        ...
        pattern_tokens.each do |token|
          if token.is_a?(PatternParser::Number::DigitToken) ||
             token.is_a?(PatternParser::Number::GroupToken) ||
             token.is_a?(PatternParser::Number::DecimalToken)
